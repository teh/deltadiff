...
def _unpack_signature(binary_signature):
    """
...
def generate_delta(path, signature, block_size=BLOCK_SIZE):
    sig_lookup = dict(
        (token, hash) for _, token, hash in _unpack_signature(signature)
    )
    data = open(path).read()
    i = 0
    start = 0
...
def _unpack_delta(binary_delta):
    i = 0
...
def patch(in_path, out_path, signature, binary_delta, block_size=BLOCK_SIZE):
    position_lookup = dict((hash, pos) for pos, _, hash in _unpack_signature(signature))

    in_data = open(in_path).read()
    out_file = open(out_path, 'w')
    for what, data_or_hash in _unpack_delta(binary_delta):
        if what == 'use':
